...
The plot settings in set by this function are to draw the LFC line at 1, and only color/label those points which are greater than Log2fc of 1 and an adjusted p-value less than .001.  

## Plots for aerobic vs anaerobic
```{r}
maPlotATM_05 <- makeMA_05(resATM_df) + ggtitle("Anaerobic vs Aerobic (ref)") +theme(
  ...
  scale_colour_manual(values = alpha(c("limegreen", "firebrick2", "gray60"),0.5), labels = c("Upregulated (262)", "Downregulated (104)", "NS (15,873)"))
...
maPlotATM_01 <- makeMA_01(resATM_df) + ggtitle("Anaerobic vs Aerobic (ref)") +theme(
    ...
    legend.position = "top") +
  guides(colour = guide_legend(override.aes = list(alpha=0.5, size=3))) +
  scale_colour_manual(values = alpha(c("limegreen", "firebrick2", "gray60"),0.5), labels = c("Upregulated (204)", "Downregulated (72)", "NS (15,963)"))
maPlotATM_01
ggsave(plot = maPlotATM_01, filename = "../figures/anaVSaer/maPlotATM_aerREF_01.pdf", device = "pdf", height = 4, width = 6)
```  

We can look at these two genes by using the plotCounts function: 

```{r}
# Find the ensemble id (ensgene) for LDHA in the resATM_df file
HIF1A <- "ENSG00000100644"
# Save plotcounts to a data frame object
HIF1A <- plotCounts(dds, gene=HIF1A, intgroup="TEST", returnData=TRUE)
# Plotting the LDHA normalized counts, using the samplenames (rownames of plot as labels)
ggplot(HIF1A, aes(x = TEST, y = count, color = TEST)) + 
  geom_boxplot() +
  geom_point(position=position_jitter(w = 0.1,h = 0)) +
  scale_color_manual(limits = c("aerobic", "anaerobic", "anaerobic_CRS"), 
                     values = wes_palette(n=3, name="Darjeeling1")) +
  geom_text_repel(aes(label = rownames(HIF1Am))) + 
  ggtitle("HIF1A") +
